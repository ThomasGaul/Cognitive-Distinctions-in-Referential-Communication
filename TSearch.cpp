...
TSearch::TSearch(int VSize, double (*EvalFn)(TVector<double> &, RandomState &))
{
	...
	WritePopulationVectorFunction = NULL;
...
void TSearch::DoSearch(int ResumeFlag)
{
	...
	if (UpdateBestFlag && BestActionFunction != NULL)
		(*BestActionFunction)(Gen,bestVector,BestPerf);
	// Repeat until done
	while (!SearchTerminated())
	{
		...
		if (WritePopulationVectorFunction != NULL)
			(*WritePopulationVectorFunction)(Perf, Gen);
		...
		if ((PopVectInt > 0) && (Gen > 0) && ((Gen % PopVectInt) == 0) && (WritePopulationVectorFunction != NULL))
			(*WritePopulationVectorFunction)(Perf, Gen);
	}
	// Display results
...
{
	// Check for fluke
	if (ReEvalFlag && (*SearchTerminationFunction)(Gen,BestPerf,AvgPerf,PerfVar))
		BestPerf = EvaluateVector(bestVector,RandomStates[0]);
